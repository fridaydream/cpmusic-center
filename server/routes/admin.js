...
class adminController {
  ...
  async createMusic(ctx, next) {
    ...
    console.log('url, cover, author, name', url, cover, author, name);
    const result = await createMusic({ url, cover, author, name });
    if (result) {
      ctx.body = {
        success: true
      }
  ...
  async updateMusic (ctx, next) {
    ...
    await updateMusicById({ id, url, cover, author, name });
    ctx.body = {
      success: true
    }
  }
